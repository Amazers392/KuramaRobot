...
import Kurama.modules.sql.connection_sql as sql
from Kurama import dispatcher, SHINOBI, SAGE
from Kurama.modules.helper_funcs import chat_status
...
def connect_chat(update, context):
    ...
    if update.effective_chat.type == "private":
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in SHINOBI):
...
def connected(bot: Bot, update: Update, chat, user_id, need_admin=True):
    ...
    if chat.type == chat.PRIVATE and sql.get_connected_chat(user_id):
        ...
        if ((isadmin) or (isallow and ismember) or (user.id in SHINOBI) or
            (user.id in SAGE)):
            if need_admin is True:
                if (getstatusadmin.status in ("administrator", "creator") or
                        user_id in SHINOBI or user.id in SAGE):
                    return conn_id
                else:
                    send_message(
                        update.effective_message,
